	...
	safe_ctrl::safe_ctrl(ros::NodeHandle& nh, ros::NodeHandle& pnh) :nh_(nh)
	{
        ...
        {    
            if (NavisNewMsg)
            {
                ...
                double ax_r = gain_cmd * (cmd_geo_x.x + cmd_geo_y.x - vel_current.x);
                double ay_r = gain_cmd * (cmd_geo_x.y + cmd_geo_y.y - vel_current.y);

                output_cmd_idea.x = input_cmd.linear.x + ax_r*cos(euler.z)+ay_r*sin(euler.z);
                output_cmd_idea.y = input_cmd.linear.y - (-ax_r*sin(euler.z)+ay_r*cos(euler.z)); // linear.y<0, fly to right
